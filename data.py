...
def load_dataset(filename):
    ...
    # sensor numbering
    sensorsList = sorted(set(sensors)) # unique sensor names
    dictSensors = {}
    ...
    # value numbering
    valueList = sorted(set(values)) # unique values
    dictValues = {}
    for i, v in enumerate(valueList):
        dictValues[v] = i

    # embedding for all unique values of multi-variate sensors
    dictObs = {}
    count = 0
    for key in dictSensors.keys(): # unique sensors
        if "M" or "AD" in key: # binary numbering
